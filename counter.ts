...
type ActionName = "INCREMENT" | "DECREMENT" | "RESET" | "ADD_FOUR" | "DOUBLE";
type ActionCode = 1 | 2 | 3 | 4 | 5;

const ACTION: Readonly<Record<ActionName, ActionCode>> = Object.freeze({
  INCREMENT: 1,
  DECREMENT: 2,
  RESET: 3,
  ADD_FOUR: 4,
...
document.getElementById("counter")!.addEventListener("click", function (event) {
  var action = (event.target as HTMLElement).dataset.action as ActionName | undefined;
  if (!action) {
    return;
  }
...
function dispatch(x: ActionCode): void {
  // main logic
  switch (x) {
